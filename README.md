...
This library provides convenient access to the Demo API (Scalar Galaxy) from Ruby.
...
| `o_auth2` | `String \| nil` | - | OAuth 2.0 authentication Defaults to O_AUTH2. |
| `open_id_connect` | `String \| nil` | - | OpenID Connect Authentication Defaults to OPEN_ID_CONNECT. |
...
begin
  ...
  puts response.inspect
rescue DemoApiScalarGalaxy::Errors::APIError => error
...
| `o_auth2` | `String \| nil` | `ENV["O_AUTH2"]` | OAuth 2.0 authentication |
| `open_id_connect` | `String \| nil` | `ENV["OPEN_ID_CONNECT"]` | OpenID Connect Authentication |
| `webhook_secret` | `String \| nil` | `ENV["OFFICIAL_GALAXY_TESTING_WEBHOOK_SECRET"]` | Secret used to verify incoming webhook signatures. |
| `environment` | `:production \| :responds_with_your_request_data \| nil` | - | Environment to target; each one maps to a different base URL. |
| `base_url` | `String \| nil` | `ENV["OFFICIAL_GALAXY_TESTING_BASE_URL"]` | Override the default API base URL. |
